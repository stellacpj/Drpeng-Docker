# Version: 0.0.1
FROM ubuntu:trusty
MAINTAINER Stella "stella@juniper.net"
...
RUN apt-get -yqq install python-jinja2
...
RUN git clone https://github.com/stellacpj/JunosPortal.git && \
	chown -R www-data:www-data JunosPortal && \ 
        mv JunosPortal web2py/applications/JunosPortal 

USER www-data

WORKDIR $W2P_DIR

RUN python -c "from gluon.main import save_password; save_password('admin',80)" && \
	python -c "from gluon.main import save_password; save_password('admin',443)"
